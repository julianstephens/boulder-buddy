...
export const RoutineModal = () => {
  ...
  const submit = async (data: Prisma.RoutineCreateInput) => {
    ...
    const formData = {
      name: data.name,
      ...(!isEmpty(data.duration) && data.duration > 0 ? { duration: data.duration } : {}),
    ...
    >
      <Form className="full" form={form} onSubmit={submit}>
        <label htmlFor="name" className="w-full">
          <Required name="Name" />
          <Input type="text" className="w-full" id="name" name="name" register={form.register} required />
        </label>
        <label htmlFor="duration" className="w-full">
          Duration (minutes)
          <Input type="number" className="w-full" id="duration" name="duration" register={form.register} />
        </label>
        <label htmlFor="note" className="w-full">
          Note
          <Textarea type="text" rows={5} className="w-full" id="note" name="note.create.text" register={form.register} />
        </label>
      </Form>
    </AppModal>
  );
